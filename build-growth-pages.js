 ...
 * GrowthAtlas.com programmatic page generator.
 *
...
function header() {
  ...
  <a href="${SITE}" class="header-logo">GrowthAtlas.com</a>
...
const FOOTER = `<footer>
  <a href="${SITE}">GrowthAtlas.com</a> &bull;
  <a href="${SITE}/all-growth-etfs.html">All GrowthAtlas</a> &bull;
  ...
  GrowthAtlas.com is for entertainment &amp; educational purposes ONLY.
</footer>`;

const DISCLAIMER = `<p class="fine"><strong>Disclaimer:</strong> GrowthAtlas.com is for informational and educational
...
function etfPage(etf, all) {
  ...
  if (rank === null) {
    ...
    placement = `${esc(etf.symbol)} currently sits ${ordinal(rank)} out of ${total} funds on the GrowthAtlas list when sorted by total return.`;
  ...
  if (etf.aumNum === null) {
    ...
    sizeNote = `With ${esc(etf.aum)} in assets, ${esc(etf.symbol)} clears the $1B mark used by the "Big AUM" filter on the main GrowthAtlas list.`;
  ...
  const faq = [
    {
      ...
      a: `${etf.symbol} is ${etf.name}. It is one of ${total} growth-focused ETFs tracked on the GrowthAtlas.com curated list.`
    },
    {
    ...
    (cls.leveraged || cls.inverse)
      ...
      : {
          ...
          a: `We don't answer that. GrowthAtlas.com is an informational and entertainment site and publishes no buy or sell signals. Whether any fund suits you depends on your own goals, time horizon and risk tolerance. Speak with a licensed financial advisor.`
        }
  ];
  ...
  const crumbSchema = {
    ...
    '@type': 'BreadcrumbList',
    itemListElement: [
      { '@type': 'ListItem', position: 1, name: 'GrowthAtlas.com', item: SITE + '/' },
      { '@type': 'ListItem', position: 2, name: 'All GrowthAtlas', item: SITE + '/all-growth-etfs.html' },
      { '@type': 'ListItem', position: 3, name: etf.symbol, item: url }
    ]
  };

  const title = `${etf.symbol} \u2014 ${etf.name} | Total Return & AUM | GrowthAtlas.com`;
  const desc  = `${etf.symbol} (${etf.name}): total return ${etf.returnVal}, AUM ${etf.aum}${rank !== null ? `, ranked ${ordinal(rank)} of ${total}` : ''} on the GrowthAtlas.com curated growth ETF list.`;
...
${header()}
<main class="wrap">
  <div class="crumbs"><a href="${SITE}/">Home</a> &rsaquo; <a href="${SITE}/all-growth-etfs.html">All GrowthAtlas</a> &rsaquo; ${esc(etf.symbol)}</div>
...
function hubPage(all) {
  ...
  const title = `All GrowthAtlas \u2014 Full List of ${all.length} Funds | GrowthAtlas.com`;
  const desc = `Complete list of all ${all.length} growth ETFs tracked on GrowthAtlas.com, with total return and AUM for each. Free, updated daily.`;
...
${header()}
<main class="wrap">
  <div class="crumbs"><a href="${SITE}/">Home</a> &rsaquo; All GrowthAtlas</div>

  <h2 style="margin-top:16px">All ${all.length} growth ETFs we track</h2>
  <p>Every growth ETF on the GrowthAtlas.com list, ranked by total return. Each ticker links to its
